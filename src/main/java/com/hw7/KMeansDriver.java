...
public class KMeansDriver {
	...
	public void KMeansClusterJod() throws IOException, InterruptedException, ClassNotFoundException{
		this.conf.set("mapreduce.output.textoutputformat.separator", ",");
		Job kMeansClusterJob = Job.getInstance(this.conf);
		kMeansClusterJob.setJobName("KMeansClusterJob");
		kMeansClusterJob.setJarByClass(KMeansCluster.class);
		
		kMeansClusterJob.getConfiguration().set("clusterPath", outputPath + "/cluster-" + iterationNum +"/");   //我感觉原来这里写错了，应该是cluster-inerNum，而不是inerNum-1，不然最后一次相当于白做了
